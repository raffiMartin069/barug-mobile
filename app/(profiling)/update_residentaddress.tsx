...
const ResidentAddress = () => {
  ...
  const submitAddress = () => {
    ...
    if (!s) { Alert.alert('Validation Error', 'Street is required.'); return; }
    if (!p) { Alert.alert('Validation Error', 'Purok or Sitio is required.'); return; }
    if (!b) { Alert.alert('Validation Error', 'Barangay is required.'); return; }
    if (!c) { Alert.alert('Validation Error', 'City is required.'); return; }

    // Save to store (this also rebuilds haddress)
    setAddress({ street: s, puroksitio: p, brgy: b, city: c });

    // Go back to UpdateProfile and tell it NOT to prefill again
    router.replace({ pathname: '/update_profile', params: { skipPrefill: '1' } });
  };
  ...
  return (
    <ThemedView safe={true}>
      ...
      <ThemedKeyboardAwareScrollView>
        <View>
          <ThemedTextInput placeholder="Street" value={streetState} onChangeText={setStreet} autoCapitalize="words" />
          <Spacer height={10} />
          <ThemedTextInput placeholder="Purok or Sitio" value={purokState} onChangeText={setPurokSitio} autoCapitalize="words" />
          <Spacer height={10} />
          <ThemedTextInput placeholder="Barangay" value={brgyState} onChangeText={setBrgy} autoCapitalize="words" />
          <Spacer height={10} />
          <ThemedTextInput placeholder="City" value={cityState} onChangeText={setCity} autoCapitalize="words" />
        </View>
